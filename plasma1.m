...
%refractive index n relative to theta from 0 to pi/2 in constant temperature T
...
T = 10; % Temperature (KeV)
f = 100e9; % Frequency (Hz)
theta = linspace(0, pi/2, 1000); % Angle range from 0 to pi/2
...
Vthe = sqrt(KB * T*1.6e-16 / me); % Thermal velocity
...
B = @(nsq) -ezz(nsq) * exx(nsq) - ezz(nsq) * eyy(nsq) * costh^2 - exx(nsq) * eyy(nsq) * sinth^2 - exy(nsq)^2 * sinth^2 ...
           - eyz(nsq) * exz(nsq) * costh^2 + sinth * costh * exy(nsq) * exz(nsq) - 2 * sinth * costh * eyy(nsq) * exz(nsq) ...
           + exz(nsq)^2 + sinth * costh * exy(nsq) * eyz(nsq);
...
figure(1);
plot(theta, sqrt(nsq_1_real), 'b', 'LineWidth', 2); hold on;
plot(theta, sqrt(nsq_2_real), 'r', 'LineWidth', 2);
xlabel('\theta (radians)');
ylabel('Re(n^2)');
title('Real Part of n relative to \theta');
legend('Mode 1', 'Mode 2');
grid on;

%Plot the imaginary parts of the two modes
%figure(2);
%plot(theta, nsq_1_imag, 'b', 'LineWidth', 2); hold on;
%plot(theta, nsq_2_imag, 'r', 'LineWidth', 2);
%xlabel('\theta (radians)');
%ylabel('Im(n^2)');
%title('Imaginary Part of n^2 vs. \theta');
%legend('Mode 1', 'Mode 2');
%grid on;

%% Verification of R, L, O, X modes

R = 1 - omegape^2/(omega*(omega + omegace));
L = 1 - omegape^2/(omega*(omega - omegace));
P = 1 - omegape^2/omega^2;
fprintf("For Temperature equal to: %.1f", T); fprintf("Kev\n");
fprintf("The R mode refractive index is: %.6f\n", sqrt(R));
fprintf("The L mode refractive index is: %.6f\n", sqrt(L));
fprintf("The O mode refractive index is: %.6f\n", sqrt(P));
%X mode, more complex
omegae = omegace^2 + omegape^2;
X = @(nsq) ((omega^2 - omegae - nsq*(omega/c)^2*Vthe^2)*(omega^2 - omegape^2) - omegape^2*omegace^2)...
            /(omega^2*(omega^2 - omegae - nsq*(omega/c)^2* Vthe^2)) - nsq;
n0 = 1;
options = optimoptions('fsolve', 'Display', 'off');
nsq_solution = fsolve(X, n0, options);
fprintf("The X mode refractive index is: %.6f\n", sqrt(nsq_solution));